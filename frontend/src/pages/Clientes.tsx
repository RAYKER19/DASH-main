...
export default function ClientesPage({ activeView = 'clientes', onSelectView = () => undefined }: PageProps) {
  ...
  return <AppLayout activeView={activeView} onSelectView={onSelectView}>
    ...
    {tab === 'lista' && <>
      ...
      <form className="panel config-form" onSubmit={save}><h3>{editingId === null ? 'NUEVO CLIENTE' : `EDITAR CLIENTE #${editingId}`}</h3><input required placeholder="Nombre" value={form.nombre} onChange={(event) => setForm({ ...form, nombre: event.target.value })} /><input required type="email" placeholder="Email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /><input required placeholder="Teléfono" value={form.telefono} onChange={(event) => setForm({ ...form, telefono: event.target.value })} /><input required placeholder="Empresa" value={form.empresa} onChange={(event) => setForm({ ...form, empresa: event.target.value })} /><button type="submit" className="mini-btn">{editingId === null ? 'Guardar cliente' : 'Guardar cambios'}</button>{editingId !== null && <button type="button" className="mini-btn" onClick={() => { setEditingId(null); setForm(emptyForm); }}>Cancelar</button>}{message && <span className="card-detail">{message}</span>}</form>
      <section className="panel table-panel"><div className="panel-header"><h3>LISTA DE CLIENTES</h3><ExportMenu onExport={exportar} /></div><table className="data-table"><thead><tr><th>Cliente</th><th>Email</th><th>Teléfono</th><th>Empresa</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{clientes.map((client) => <tr key={client.id}><td>{client.name}</td><td>{client.email || 'Sin email'}</td><td>{client.telefono || 'Sin teléfono'}</td><td>{client.company}</td><td><span className={`status-pill ${client.status.toLowerCase()}`}>{client.status}</span></td><td><button type="button" className="mini-btn" onClick={() => { setEditingId(client.id); setForm({ nombre: client.name, email: client.email, telefono: client.telefono ?? '', empresa: client.company }); }}>Editar</button><button type="button" className="mini-btn danger-btn" onClick={() => remove(client.id)}>Eliminar</button></td></tr>)}</tbody></table>{clientes.length === 0 && <div className="empty-state"><h4>No hay clientes registrados</h4><p>Los nuevos registros de Supabase aparecerán aquí.</p></div>}</section>
    </>}
